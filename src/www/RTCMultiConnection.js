...
window.RTCMultiConnection = function(roomid, forceOptions) {
    ...
    function SocketConnection(connection, connectCallback) {
        ...
        // connection['avatar'] = being_id.avatar
        // connection['username'] = being_id.username
    ...
    (function(connection) {
        ...
        connection.open = function(localUserid, isPublicModerator,  token) {
            var oldUserId = connection.userid;
            connection.userid = connection.sessionid = localUserid || connection.sessionid;
            connection.userid += '';
            ...
            connectSocket(function() {
                ...
                if (isPublicModerator == true) {
                    connection.becomePublicModerator(token, roomid);
                }
            });
            ...
            //change so users cannot auto call in \/
            connection.captureUserMedia(typeof isPublicModerator === 'function' ? isPublicModerator : null);
        };

        connection.becomePublicModerator = (token) => { //This function checks if the socket belongs to the room creator. if it does. it returns false and ends. if not it emits a socket to the signal server
            if (!connection.isInitiator) return;
            connection.socket.emit('become-a-public-moderator', token, roomid);
        };

        connection.dontMakeMeModerator = function(token) { //Removes moderator status
        ...
        //This is the main configuration for connection.join(). Everything should be configured in here
        connection.join = connection.connect = function(remoteUserId, options, token ) {
            ...
            // message is where the data is relayed to the server on joinARoom
            var connectionDescription = {
                token: token,
                remoteUserId: connection.sessionid,
                message: {
